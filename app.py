...
app = Flask(__name__)
app.config.from_mapping({"DEBUG": True, "CACHE_TYPE": "simple", "CACHE_DEFAULT_TIMEOUT": 5400})
cache = Cache(app)


def get_stream_links(self, file_id, base_url):
        import urllib.parse
        import base64
        ...
        def get_access_link(link, cookie):
            ...
            return '{0}/play_stream?url={1}&cookie={2}'.format(base_url, link, cookie)
...
def play_stream():
    ...
    return Response(r.iter_content(chunk_size=10*1024), status_code, headers=resHeaders)
